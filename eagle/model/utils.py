...
class Timer:
    ...
    def __exit__(self, exc_type, exc_value, traceback):
        torch.cuda.synchronize()
        self.latency = (time.perf_counter_ns() - self.start)/1e3
        if(self.printing):
            # print(f'{self.name} took {self.latency:.0f} us')
            print(f"{self.latency:.0f}")
...
def initialize_tree(input_ids, model, past_key_values, logits_processor):
    ...
    draft_tokens, retrieve_indices,tree_mask, tree_position_ids, selected_tokens = model.ea_layer.topK_genrate(hidden_states, input_ids, model.base_model.lm_head, logits_processor)
    
    return draft_tokens, retrieve_indices, tree_mask, tree_position_ids, orig, hidden_states, selected_tokens
...
def update_inference_inputs(
    ...
    for past_key_values in past_key_values_list:
        for idx in range(2):
            tgt = past_key_values[idx].data[..., select_indices.to(past_key_values[idx].data.device), :]
            # Destination tensor where the relevant past information will be stored
            dst = past_key_values[idx].data[..., prev_input_len: prev_input_len + tgt.shape[-2], :]
            # Copy relevant past information from the source to the destination
            dst.copy_(tgt, non_blocking=True)
        
            past_key_values[idx].h2o_after_verifying(select_indices)
    ...
    draft_tokens, retrieve_indices, tree_mask, tree_position_ids, selected_tokens = model.ea_layer.topK_genrate(accept_hidden_state_new,
                                              input_ids=torch.cat((input_ids, token.to(input_ids.device)), dim=1),
                                              head=model.base_model.lm_head, logits_processor=logits_processor)

    new_token += accept_length + 1

    return input_ids, draft_tokens, retrieve_indices, tree_mask, tree_position_ids, new_token, None, token, selected_tokens
