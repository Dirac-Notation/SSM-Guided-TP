...
class EaModel(nn.Module):
    ...
    def __init__(
        ...
        try:
            ...
            bias=True
        self.ea_layer = eagle_model_type(config,bias=bias,total_tokens=total_token,depth=depth,top_k=top_k,threshold=threshold)
    ...
    def eagenerate(
        ...
        if hasattr(self, "past_key_values"): #False
        ...
        with Timer("all") as timer_decoding:
            ...
            for idx in range(max_length):
                ...
                with Timer("tree_decoding") as timer_ltm:
                ...
                ltm_latency.append(timer_ltm.latency)
                ssm_latency.append(timer_ssm.latency)
        ...
        return input_ids, new_token, idx+1, count_accept/count_all, count_accept/(idx+1), (input_len/timer_init.latency, new_token/timer_decoding.latency, sum(ltm_latency)/len(ltm_latency), sum(ssm_latency)/len(ssm_latency))
    ...
    def naivegenerate(
        ...
        if not log:
            return input_ids, new_token, 1.0, 1.0, starter.elapsed_time(ender)
        else:
