...
parser.add_argument('--basepath', type=str, default='/home/smp9898/.cache/huggingface/hub/models--meta-llama--Llama-2-7b-chat-hf/snapshots/f5db02db724555f92da89c216ac04704f23d4590')
...
if accelerator.is_main_process:
    ...
    wandb.init(project="two_layer", config=train_config, resume="allow")
...
def compute_loss(target, target_p, predict, loss_mask):
    ...
    vloss = torch.sum(torch.mean(loss_mask * vloss, 2)) / (loss_mask.sum() + epsilon)

    out_head = head(predict)
    
    # Forward KL divergence
    # out_logq = nn.LogSoftmax(dim=2)(out_head)
    # fkldiv = -(target_p * out_logq)
    # ploss = torch.sum(torch.sum(loss_mask * fkldiv, 2)) / (loss_mask.sum() + epsilon)
    
    # Reverse KL Divergence
    out_q = nn.Softmax(dim=2)(out_head)
    out_logqp = torch.log(out_q/(target_p + epsilon) + epsilon)
    rkldiv = out_q * out_logqp
    ploss = torch.sum(torch.sum(loss_mask * rkldiv, 2)) / (loss_mask.sum() + epsilon)
    
    # JS Divergence
    # out_p = nn.Softmax(dim=2)(out_head)
    # M = 0.5 * (out_p + target_p)
    # kl_pm = out_p * (torch.log(out_p) - torch.log(M))
    # kl_qm = target_p * (torch.log(target_p) - torch.log(M))
    # js_div = 0.5 * (kl_pm + kl_qm)
    # ploss = torch.sum(torch.sum(loss_mask * js_div, 2)) / (loss_mask.sum() + epsilon)
